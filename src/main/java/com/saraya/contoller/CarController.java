...
public class CarController {
	...
	public String carAdded(ModelMap model, @Valid @ModelAttribute("cars") Car c, BindingResult result) {
		if(result.hasErrors())
			return "addCar";
	...
	public String updateCarForm(ModelMap model, @RequestParam int id) {
		model.addAttribute("cars", cs.findById(id));
		return "addCar";
	}
	//-------------------------------------------------
	@RequestMapping(value="/update-car", method=RequestMethod.POST)
	public String carListUpdate(ModelMap model, @Valid  @ModelAttribute("cars") Car c,
			BindingResult result) {
		//creation d'error  -> fghjjkjk
